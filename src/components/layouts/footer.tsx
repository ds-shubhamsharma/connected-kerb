...
const Footer = (props: any, social: any) => {
  return (
    <>
      <footer className="site-footer">
        <div className=" centered-container ">
          <div className="backto-top">
            {" "}
            <button className="" onClick={moveTop}>
              {" "}
              {StaticData.BacktoTop}{" "}
            </button>{" "}
          </div>
          <div className="flex flex-wrap">
            <div className="logo-and-links">
              <svg
                className="connected-kerb-logo h-[40px] pr-96"
              ...
              <div className="social-media">
                {props.c_socials.map((social: any) => (
                  <>
                    <Link
                      className="social"
                      // href={social.alternateText}
                      href="#"
                      eventName={`footer`}
                    >
                      <img src={social.url} className="" alt="" />
                    </Link>
                  </>
                ))}
                <Link
                  className="primary-cta letsconnect"
                  href={props.cta.link}
                  eventName={`cta`}
                  style={{ height: "2.75rem", width: "11.75rem" }}
                >
                  {props.cta.label}{" "}
                  <svg
                    ...
                    width="17.2"
                    height="9.464"
                    viewBox="0 0 17.2 9.464"
                  >
                    <path
                      id="arrow_3_"
                      data-name="arrow (3)"
                      d="M4,12.733a.7.7,0,0,0,.717.676H18.753l-3.076,2.9a.649.649,0,0,0,0,.956.747.747,0,0,0,1.014,0l4.3-4.057a.651.651,0,0,0,.209-.475v0a.627.627,0,0,0-.056-.258.668.668,0,0,0-.155-.219L16.689,8.2a.747.747,0,0,0-1.013,0,.649.649,0,0,0,0,.956l3.077,2.9H4.717A.7.7,0,0,0,4,12.733Z"
            ...
            <div className="copyright-text">
              {props.prop &&
                props.prop.map((i: any) => {
                  return (
                    <>
                      {" "}
                      <div className="links">
                        <Link href="#" eventName={`footer`}>
                          {i.label}
                        </Link>
                      </div>
                    </>
        ...
        <div className="data">
          ...
          >
            <p className="py-2 text-left px-72">{cookieText}</p>
            <p className="py-2 text-left px-72"> {cookieText1} </p>
            <p className="py-2 text-left px-72">
              {" "}
              {cookieText2}
              <Link
                href={cookiesUrl}
                className="text-[#0091ae]"
                eventName={`cookies`}
              >
                {" "}
                {cookieText3}
            ...
            <p className="py-2 text-left px-72">{cookieText4}</p>.
            <p className="text-[#93c01f] cursor-pointer hover:underline">
              {cookieText5}
            </p>
          </CookieConsent>
        </div>
