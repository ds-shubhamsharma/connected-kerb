...
// let infoWindow.current: any = null;
...
function UnwrappedGoogleMaps({
    ...
    google.maps.event.addListener(
      ...
      function () {
        ...
        locationResults.map((result: any, index: any) => {
          ...
          for (let index = 0; index < resultelement.length; index++) {
            resultelement[index].classList.remove("fixed-hover", "active");
          }
        });
        //  map.setZoom(8);
        // infoWindow.current.close();
  ...
  function gridclick(
    ...
    let elements = document.querySelectorAll(".result");
    for (let index = 0; index < elements.length; index++) {
      if (!elements[index].classList.contains("eventbind")) {
        elements[index].classList.add("eventbind");
  ...
  function Infowindow(i: Number, result: any): void {
    ...
    const MarkerContent = (
      <>
        {" "}
        <div className="flex flex-col max-w-[24rem] w-64 lg:w-[24rem] font-main-font text-xs sm:text-sm lg:text-base">
          <div className="location-name-miles icon-row">
            ...
            <h2>
              <a className="inline-block notHighlight text-black" href={url}>
                {result.rawData.name}
              </a>
            </h2>
            {result.distance ? (
